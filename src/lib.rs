    ...
    pub fn try_get_compiler(&self) -> Result<Tool, Error> {
        ...
        match cmd.family {
            ...
            ToolFamily::Gnu | ToolFamily::Clang => {
                ...
                cmd.push_cc_arg("-ffunction-sections".into(), self.cuda);
                cmd.push_cc_arg("-fdata-sections".into(), self.cuda);
                if self.pic.unwrap_or(!target.contains("windows-gnu")) {
                    cmd.push_cc_arg("-fPIC".into(), self.cuda);
                }
            }
        }
        ...
        if self.get_debug() {
            ...
            cmd.push_cc_arg(debug_flag, self.cuda);
        }
        ...
        if self.cpp {
            match (self.cpp_set_stdlib.as_ref(), cmd.family) {
                ...
                (Some(stdlib), ToolFamily::Clang) => {
                    cmd.push_cc_arg(format!("-stdlib=lib{}", stdlib).into(), self.cuda);
                }
                _ => {
                    println!(
        ...
        if self.warnings {
            for flag in cmd.family.warnings_flags().iter() {
                cmd.push_cc_arg(flag.into(), self.cuda);
            }
        }
        ...
        if self.warnings_into_errors {
            ...
            cmd.push_cc_arg(warnings_to_errors_flag, self.cuda);
        }
...
impl Tool {
    ...
    pub fn push_cc_arg(&mut self, flag: OsString, cuda: bool) {
        if cuda {
